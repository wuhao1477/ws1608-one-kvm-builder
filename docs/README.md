...
| HCODEC 候选 | GitHub Actions `34345081710` 的 `run-32-1` 在稳定 One-KVM 用户空间完成 640×480 MMAP 30 帧编码和独立解码，60 秒健康记录完整；探针后设备仍需重启恢复 SSH，稳定性验收未通过 |
| 候选后端 | `h264_v4l2m2m` |
...
4. HCODEC 工作先验证 ARMv7 内核、DTB、固件和独立 V4L2 码流，不修改稳定资产；`run-32-1` 的 30 帧编码与持久化日志通过，但探针后设备稳定性仍未通过。
