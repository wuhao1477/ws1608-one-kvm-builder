...
- 新不可变流程完整构建、artifact 回读、镜像复验和 draft 发布：[29697101081](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29697101081)。
- 新格式 Release 识别和无更新跳过：[29697714162](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29697714162)，日志显示 `existing=ws1608-one-kvm-0.2.4-v260709-173450`。
- 迁移前旧流程最终成功构建：[29692576405](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29692576405)，只作历史排障证据。

当前 Release、四个资产摘要、输入摘要和硬件边界记录在 [HANDOFF.md](HANDOFF.md)。
...
```sh
gh api 'repos/wuhao1477/ws1608-one-kvm-builder/releases?per_page=100' \
  --jq '[.[] | select(.draft == true) | {tag_name, created_at}]'
```

正常结果为 `[]`。2026-07-20 完整构建和后续跳过检查完成后，实际查询结果为 `[]`。
