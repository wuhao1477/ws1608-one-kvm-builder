...
- Build 成功后，查看同名 Release 的四个资产均为 `uploaded`，并核对 `manifest.json` 和 `SHA256SUMS`。
...
- 强制覆盖同一 Release：[29692576405](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29692576405)。
...
强制模式会覆盖同名 Release 资产，且由于 apt 仓库和 ext4 时间戳是动态的，生成的 SHA-256 可能改变。覆盖前保存旧 Release 的 `SHA256SUMS`，并在运行完成后重新核对。
...
4. 新 Release tag 格式为 `ws1608-one-kvm-<upstream-tag>`。
...
2. 修复代码并提交到 `main`。
3. 对同一上游 tag 使用 `force=true`，让流程覆盖同名资产。
4. 重新检查 Release 是否为非 draft、四个资产是否 uploaded。
...
- [ ] Release 四个资产均为 uploaded，manifest 的 image hash 与 GitHub asset digest 相同。
