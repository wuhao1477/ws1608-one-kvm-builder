...
- 旧版强制覆盖参考运行：[29692576405](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29692576405)。新版强制构建使用新的 `bNNN` tag，不覆盖该 Release。
...
强制模式会递增构建序号并创建新的 draft Release，所有资产和摘要复验通过后才公开。由于 apt 仓库和 ext4 时间戳是动态的，新构建的 SHA-256 可能改变；旧 Release 保持不变。
...
4. 新 Release tag 格式为 `ws1608-one-kvm-<one-kvm-rust-version>-<upstream-tag>-bNNN`。
...
2. 修复代码并提交到 `main`。
3. 对同一上游输入使用 `force=true`，让流程创建下一个 `bNNN`。
