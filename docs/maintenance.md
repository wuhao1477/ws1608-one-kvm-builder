...
- 没有新 tag 时必须看到 `No new One-KVM input`，且 build job 是 skipped。
- Build 成功后，查看 artifact 的五个文件均存在；Release 公开后核对 `.burn.img`、`.burn.img.xz`、`SHA256SUMS`、`manifest.json` 和 `validation-report.json`。
...
- 旧版强制覆盖参考运行：[29692576405](https://github.com/wuhao1477/ws1608-one-kvm-builder/actions/runs/29692576405)。新版强制构建使用新的 `bRRRAAA` tag，不覆盖该 Release。
...
强制模式会使用当前 Actions run number 和 attempt 创建新的 draft Release，所有资产和摘要复验通过后才公开。由于 apt 仓库和 ext4 时间戳是动态的，新构建的 SHA-256 可能改变；旧 Release 保持不变。
...
4. 新 Release tag 格式为 `ws1608-one-kvm-<one-kvm-rust-version>-<upstream-tag>-bRRRAAA`。
...
5. 修改 `config/base.env` 的资产字段和 `BASE_ID`、`BASE_FLAVOR`、`BASE_KERNEL`、`BASE_BOARD`；这些字段共同决定下载校验、文件名和 manifest。
6. 搜索并检查硬编码的版本文字：
...
2. 修复代码并提交到 `main`。
3. 对同一上游输入重新 dispatch `force=true`，让流程创建新的 `bRRRAAA`。
4. 重新检查新 Release 是否为非 draft、五个资产是否 uploaded，以及 manifest 和报告摘要。

基础 Release 应使用新 tag，不要覆盖已有基础资产；覆盖基础包会让历史 manifest 的 URL 指向不同内容。

## 依赖和权限

- GitHub Actions 默认只有 `contents: read`；仅 `release` job 使用 `contents: write`，pull request 不发布。
- `actions/checkout`、`actions/setup-go`、`actions/upload-artifact` 和 `actions/download-artifact` 均固定完整 commit SHA；升级时重新审查 SHA 和 runtime 兼容性。
...
OUTPUT_DIR=release-check IMAGE_NAME="$IMAGE_NAME" ./scripts/verify-release-assets.sh
```
...
- [ ] Release 五个资产均为 uploaded，`SHA256SUMS`、manifest 和 validation report 复验通过。
