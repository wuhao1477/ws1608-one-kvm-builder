...
function fixture(t, order) {
  ...
  executable(path.join(bin, 'mkimage'), '#!/bin/sh\ncase "$2" in *uImage.amlenc*) echo Linux-3.10.107-WS1608-AMLENC ;; *uInitrd.amlenc*) echo Linux-3.10.107-WS1608-AMLENC-initrd ;; *boot.scr*) echo WS1608-AMLENC-Bringup ;; esac\n');
  ...
  executable(path.join(bin, 'debugfs'), [
    ...
    '  "stat /etc/init.d/ws1608-amlenc-firstboot"|"stat /usr/local/sbin/ws1608-amlenc-"*|"stat /usr/local/libexec/ws1608-amlenc/"*) echo "Inode: 42 Mode:  0755" ;;',
    '  "stat /usr/local/lib/ws1608-amlenc/libvpcodec.so"|"stat /usr/local/share/ws1608-amlenc/frame-"*|"stat /usr/local/share/ws1608-amlenc/hardware-limits.json") echo "Inode: 42 Mode:  0644" ;;',
