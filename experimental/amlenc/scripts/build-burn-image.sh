...
SOFTWARE_CODECS_RUNTIME_VERIFIED=${SOFTWARE_CODECS_RUNTIME_VERIFIED:?SOFTWARE_CODECS_RUNTIME_VERIFIED is required}
...
PACKAGE_METADATA_DIR="$WORK_DIR/one-kvm-metadata"
...
[[ "$SOFTWARE_CODECS_RUNTIME_VERIFIED" == true ]] || fail "software codec runtime gate has not passed"
...
rm -rf "$STAGING_DIR" "$PACKAGE_DIR" "$PACKAGE_METADATA_DIR"
rm -f "$OUTPUT_DIR/$IMAGE_NAME" "$MANIFEST"
mkdir -p "$STAGING_DIR" "$PACKAGE_METADATA_DIR"
