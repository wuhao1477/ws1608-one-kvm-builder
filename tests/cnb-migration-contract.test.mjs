...
test('HCODEC runner builds and exports the pinned AmlImg tool before base evidence', () => {
  ...
  assert.match(runner, /apt-get install -y e2fsprogs mtools xz-utils gcc-arm-linux-gnueabihf binutils-arm-linux-gnueabihf build-essential python3/);
...
test('HCODEC runner uses the CNB Docker-in-Docker permissions required by Armbian', () => {
  ...
  assert.match(runner, /-e KERNEL_GIT=shallow/);
